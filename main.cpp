...
int main()
{
	std::shared_ptr<imageProcess> ptrImageProcess = std::make_shared<imageProcess>();
	for (int i = 1; i <= 12; ++i)
	{
		//i = 10;
		cv::Mat src = cv::imread("./testPic08032000/3"+std::to_string(i)+".bmp");
		//cv::Mat src = cv::imread("C:\\Users\\19248\\MVS\\Data\\1.bmp");
		printf("/************************************ %d ***********************/ \n", i);
		if (src.channels() != 1)
		{
			cvtColor(src, src, CV_BGR2GRAY);
		}
		ptrImageProcess->imageProcessTask(src, i-1);
	}
}
